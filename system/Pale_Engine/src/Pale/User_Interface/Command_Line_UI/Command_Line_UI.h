...
namespace Pale::UI {
	...
	public:
		...
		Command_Line_UI(std::shared_ptr<Chess_Logic::Board_Representation<std::string>> boardRepresentation, unsigned short int numberAI, APP_STATES applicationState) 
			: User_Interface(boardRepresentation), _applicationState(applicationState), _drawing(true), _insertMoveCommand(false), _skipUserInput(false), _numberOfAIInstances(numberAI), _printHelp(false), _invalidCommandInserted(false), _turnCounter(1), _resetEnvironment(false) {}
		~Command_Line_UI() = default;

		void Draw() override;
		void Update(std::shared_ptr<Chess_Logic::Board_Representation<std::string>> boardRepresentation, APP_STATES applicationState, unsigned short int turn) override;
	...
	private:
		...
		bool _drawing, _skipUserInput, _insertMoveCommand, _printHelp, _invalidCommandInserted, _resetEnvironment;
